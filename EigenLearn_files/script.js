...
1. Let $ (f_n)_{n \in \mathbb{N}} $ be a sequence of nonnegative measurable functions over $ A \subseteq \mathbb{R} $. Prove that if $ f_1 $ is integrable over $A$ and $f_{n+1} < f_n$ $\forall n \in \mathbb{N}$, then:$$  \lim_{n \rightarrow \infty} \int_A f_n = \int_A \lim_{n \rightarrow \infty} f_n $$
...
1. Define the functions $f$ and $g$ on $[0,]$ by $f(x) := x^{1/3}$ for $0 \leq x \leq 1$ and:
$$
g(x) := \begin{cases}
...
\item We define the convolution $f * g$ of $f$ and $g$ as: 
$$
...
3. Prove that $f * g$ is integrable whenever $f$ and $g$ are integrable, and that
$$
...
where the infimum is taken over all finite coverings of $A$ by open intervals $I_k$. The function $m_J^*$ s called the outer Jordan measure. 
...
<question-env>
**(Math 454, Assignment #3)** Let $(f_n)$ be a sequence of measurable functions defined on a measurable set $A \subseteq \mathbb{R}$. Let $A_0$ be the set of points $x$ in $A$ at which $(f_n(x))$ converges. Is the set $A_0$ measurable? Hint: Recall that a sequence in $\mathbb{R}$ converges $\iff$ it's Cauchy.
</question-env> 
...
Hint for (2): Construct an open set $A$ which contains $\mathbb{Q} \cap [0,1]$ while $m(A) < 1/2$. 
</question-env> 
...
<question-env> 
**(Math 454, Assignment #3)**
Let $f$ and $g$ be two bounded, measurable functions defined on a set of finite measure $A \subseteq A$. Prove that if $f = g$ a.e. in $A$, then $\int_A f = \int_A g$.
</question-env> 
